...
void Commands::processPacketNoProtocol(QByteArray data)
{
    VByteArray vb(data);
    emit printCmdFeedback(QString::fromLatin1(vb));
    // qDebug()<<"Commands"<<vb<<endl;
}
...
void Commands::sendSerialCmdNoProtocol(QString cmd)
{
    VByteArray vb;
    //vb.vbAppendInt8(COMM_CMD_SEND);
    vb.append(cmd.toLatin1());
...
void Commands::firmwareUploadUpdate(bool isTimeout)
{
    ...
    else if (mFirmwareState == 1)
    {
        ...
        if (isTimeout)
        {
            ...
            VByteArray vb;
            vb.append(mFirmwareFwdAllCan ? (char)COMM_WRITE_NEW_APP_DATA_ALL_CAN : (char)COMM_WRITE_NEW_APP_DATA);
            vb.vbAppendUint32(0);
    ...
    else if (mFirmwareState == 2)
    {
        ...
        else
        {
            ...
            if (mFimwarePtr >= mNewFirmware.size())
            {
                ...
                {
                    QByteArray buffer;
                    buffer.append(mFirmwareFwdAllCan ? (char)COMM_JUMP_TO_BOOTLOADER_ALL_CAN : (char)COMM_JUMP_TO_BOOTLOADER);
                    emitData(buffer);
                }
            }
            else
...
void Commands::startFirmwareUpload(QByteArray &newFirmware, bool isBootloader, bool fwdCan)
{
    ...
    if (mFirmwareIsBootloader)
    {
        if (mLimitedSupportsEraseBootloader)
        {
            mFirmwareState = 0;
            VByteArray vb;
            vb.vbAppendInt8(mFirmwareFwdAllCan ? COMM_ERASE_BOOTLOADER_ALL_CAN : COMM_ERASE_BOOTLOADER);
            emitData(vb);
        }
        else
        {
    ...
    {
        VByteArray vb;
        vb.vbAppendInt8(mFirmwareFwdAllCan ? COMM_ERASE_NEW_APP_ALL_CAN : COMM_ERASE_NEW_APP);
        vb.vbAppendUint32(mNewFirmware.size());
        emitData(vb);
    }
}
