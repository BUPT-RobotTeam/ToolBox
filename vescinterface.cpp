...
VescInterface::VescInterface(QObject *parent) : QObject(parent)
{
    ...
    mRxProtocolState = false;
    mTxProtocolState = false;
    ...
    connect(mPacket, SIGNAL(packetReceivedNoProtocol(QByteArray &)),
            this, SLOT(packetReceivedNoProtocol(QByteArray &)));
    ...
    connect(mCommands, &Commands::monitorReceived, [this](MONITOR_VALUES v) {
...
bool VescInterface::swdUploadFw(QByteArray newFirmware, uint32_t startAddr, bool verify)
{
    ...
    auto waitBmReadRes = [this]() {
        ...
        timeoutTimer.setSingleShot(true);
        timeoutTimer.start(3000);
        auto conn = connect(mCommands, &Commands::bmReadMemRes, [&res, &resData, &loop](int rdRes, QByteArray data) {
            res = rdRes;
            resData = data;
            loop.quit();
        });
    ...
    auto writeChunk = [this, &waitBmWriteRes, &waitBmReadRes, &verify](uint32_t addr, QByteArray chunk) {
        for (int i = 0; i < 3; i++)
        {
            ...
            if (verify && (!mCommands->isLimitedMode() ||
                           mCommands->getLimitedCompatibilityCommands().contains(int(COMM_BM_MEM_READ))))
            {
...
bool VescInterface::openRtLogFile(QString outDirectory)
{
    ...
    mRtLogFile.setFileName(QString("%1/%2-%3-%4_%5-%6-%7.csv").arg(outDirectory).arg(d.date().year(), 4, 10, QChar('0')).arg(d.date().month(), 2, 10, QChar('0')).arg(d.date().day(), 2, 10, QChar('0')).arg(d.time().hour(), 2, 10, QChar('0')).arg(d.time().minute(), 2, 10, QChar('0')).arg(d.time().second(), 2, 10, QChar('0')));
...
void VescInterface::packetReceivedNoProtocol(QByteArray &data)
{
    if (mRxProtocolState)
    {
        return;
    ...
    {
        mCommands->processPacketNoProtocol(data);
...
void VescInterface::cmdDataToSend(QByteArray &data)
{
    ...
    {
        mPacket->sendPacketNoProtocol(data);
    }
}
    ...
    else if (fw_connected > highest_supported)
    {
        ...
        if (!wasReceived)
        {
            emit messageDialog(tr("Warning"), tr("The connected VESC has newer firmware than this version of"
                                                 ...
                                                 " Until then, limited communication mode will be used."),
                               false, false);
        }
    }
...
void VescInterface::setRxProtocolState(bool rxpState)
{
    mRxProtocolState = rxpState;
}

bool VescInterface::getRxProtocolState()
{
    return mRxProtocolState;
}

void VescInterface::setTxProtocolState(bool txpState)
{
    mTxProtocolState = txpState;
}

bool VescInterface::getTxProtocolState()
{
    return mTxProtocolState;
}
